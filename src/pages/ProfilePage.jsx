...
import { readGuestStats } from "../hooks/useGuestSave";
...
function StatsGrid({ rows }) {
  ...
  if (rows.length === 0) {
    return <p className="text-slate-500 text-sm">Play a game to see scores and progress here.</p>;
  }
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {rows.map((row, index) => (
          <div key={row.key} className="rounded-xl border border-slate-100 bg-slate-50/80 p-4 shadow-sm">
...
function GuestProfilePreview() {
  const rows = useMemo(
    () => statsToEntries(readGuestStats()).sort((a, b) => (b.highScore ?? 0) - (a.highScore ?? 0)),
    []
  );
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-10 sm:py-12">
        <div className="card-elevated overflow-hidden">
          <div className="bg-gradient-to-r from-indigo-600 to-violet-600 px-6 sm:px-8 py-8 text-white">
            ...
            <h1 className="text-3xl font-bold mt-1">Guest</h1>
            <p className="text-indigo-100/90 mt-2">
              This is a local preview only — sign up to keep this progress permanently.
            </p>
          </div>

          <div className="p-6 sm:p-8">
            <SignupPrompt
              message="You're browsing as a guest. Sign up to save these scores to a real account."
              dismissible={false}
            />
            <h2 className="text-lg font-bold text-slate-900 mb-4">Game stats (saved on this device)</h2>
            <StatsGrid rows={rows} />
          </div>
        </div>
      </main>
